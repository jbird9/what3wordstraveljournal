...
words = []
syllable_len = []
(0..lats_longs.length-1).each do |j|
	url = "https://api.what3words.com/v2/reverse?coords=#{lats_longs.keys[j].to_s.insert(-8,'.')}%2C#{lats_longs.values[j].to_s.insert(-8,'.')}&key=#{w3wkey}&lang=en&format=json&display=full"
	url_str = "\'#{url}\'"
